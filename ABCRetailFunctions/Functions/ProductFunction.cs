...
public class ProductFunction
{
    ...
    [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "products")] HttpRequestData req, FunctionContext ctx)
    {
        ...
        else
        {
            ...
            desc = body.TryGetValue("", out var d) ? d?.ToString() ?? "" : "";
    ...
    [HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "products/{id}")] HttpRequestData req, string id)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                if (body.TryGetValue("ProductName", out var pn))
                    e.ProductName = pn?.ToString() ?? e.ProductName;

                if (body.TryGetValue("Description", out var d))
                    e.Description = d?.ToString() ?? e.Description;

                if (body.TryGetValue("Price", out var pr) && double.TryParse(pr.ToString(), out var price))
                    e.Price = price;

                if (body.TryGetValue("StockAvailable", out var st) && int.TryParse(st.ToString(), out var stock))
                    e.StockAvailable = stock;

                if (body.TryGetValue("ImageuRL", out var iu))
                    e.ImageURL = iu?.ToString() ?? e.ImageURL;
