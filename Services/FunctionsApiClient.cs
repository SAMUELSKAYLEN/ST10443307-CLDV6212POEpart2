...
public class FunctionsApiClient : IFunctionsApi
{
    ...
    {
        var resp = await _http.GetAsync(CustomersRoute);
        resp.EnsureSuccessStatusCode();
        return await ReadJsonAsync<List<Customer>>(resp);
    }
    ...
    {
        var resp = await _http.GetAsync($"{OrdersRoute}/{id}"); // fixed
        if (resp.StatusCode == System.Net.HttpStatusCode.NotFound) return null;
    ...
    public async Task<string> UploadProofOfPaymentAsync(IFormFile file, string? orderId, string? customerName)
    {
        ...
        return doc != null && doc.TryGetValue("fileName", out var name) ? name : file.FileName;
    }
